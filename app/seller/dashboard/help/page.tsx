...
import { Search, LifeBuoy, Wallet, Box, ShieldCheck, Tag, MessageSquare, FileText, Plus, ChevronRight } from "lucide-react";
import { supportApi, type Article } from "@/lib/supportApi";

const TOP_CATEGORIES = [
  { label: "Payouts",       icon: Wallet,       bg: "#E8F4FD", fg: "#1A6FD4", category: "Payouts" },
  { label: "Listings",      icon: Tag,          bg: "#E8F4FD", fg: "#1A6FD4", category: "Listing rejected" },
  { label: "KYC",           icon: ShieldCheck,  bg: "#E8F4FD", fg: "#1A6FD4", category: "KYC" },
  { label: "Inventory",     icon: Box,          bg: "#E8F4FD", fg: "#1A6FD4", category: "Inventory issue" },
];
...
export default function SellerHelpPage() {
  ...
  return (
    <main className="w-full mx-auto max-w-5xl px-3 sm:px-4 py-6 md:px-8 md:py-10 bg-white md:bg-transparent min-h-[calc(100vh-64px)]">
      {/* ── Desktop Header ── */}
      ...
      <div className="flex flex-col-reverse md:flex-row gap-8 md:gap-10 mb-10 md:mb-12">
        ...
        <section className="md:w-[340px] shrink-0 flex flex-col gap-3 md:gap-4">
          ...
          <Link
            ...
            className="flex items-center gap-4 rounded-2xl p-5 md:p-6 transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5 active:scale-[0.98] group cursor-pointer bg-[#1A6FD4] text-white hover:bg-[#1559B3]"
          >
            ...
            <div>
              ...
              <div className="text-[14px] text-white/80 font-medium leading-tight">Reach the seller-support team.</div>
            </div>
          </Link>
          <Link
