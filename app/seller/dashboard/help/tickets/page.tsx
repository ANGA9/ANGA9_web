...
import { ArrowLeft, MessageSquare, Plus, Inbox, Megaphone } from "lucide-react";
...
export default function SellerMyTicketsPage() {
  ...
  return (
    <main className="w-full mx-auto max-w-4xl px-3 sm:px-4 py-6 md:px-8 md:py-10 bg-white md:bg-transparent min-h-[calc(100vh-64px)]">
      <Link href="/seller/dashboard/help" className="inline-flex items-center gap-1 text-sm font-semibold text-[#1A6FD4] hover:underline mb-2">
      ...
      <section className="mt-6">
        {loading ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-gray-200 bg-white md:bg-gray-50/50 py-16 text-[15px] font-medium text-gray-400">
            <div className="h-8 w-8 animate-spin rounded-full border-2 border-[#1A6FD4] border-t-transparent mb-4" />
            Loading tickets...
          </div>
        ) : tickets.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 bg-white md:bg-gray-50/50 p-12 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-white shadow-sm border border-gray-100 mb-4">
          ...
          <ul className="space-y-3 md:space-y-4">
            {tickets.map((tk) => (
              <li key={tk.id}>
                <Link
                  href={`/seller/dashboard/help/tickets/${tk.id}`}
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0 flex-1 pr-4">
                      <div className="flex items-center gap-2 text-[12px] font-bold text-gray-400 uppercase tracking-wider mb-1">
                        ...
                        {tk.initiator_role === "admin" && (
                          <>
                            <span className="hidden md:inline-block w-1 h-1 rounded-full bg-gray-300"></span>
                            <span className="inline-flex items-center gap-1 rounded-full bg-[#EDE9FE] px-2 py-0.5 text-[10px] font-semibold text-[#7C3AED]">
